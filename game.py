...
class Game():
    ...
    def displayAverageFPS(self):
        pygame.draw.rect(self.screen,('#9e482c'),(90,90,200,300))
        Game.drawParticles(self, ('#FFFFFF'), randint(-5,5)/10 + 1, randint(0,13), randint(4,9))
    ...
    def checkFullscreen(self): 
        if self.FULLSCREEN == 1:
            ...
            self.screen = pygame.display.set_mode(((self.DISPLAY_W, self.DISPLAY_H)),pygame.NOFRAME)
    ...
    def drawParticles(self, color, initial_x_velocity, initial_y_velocity, radius):
        self.particles.append([[self.Mouse_x, self.Mouse_y], [initial_x_velocity*-1, initial_y_velocity*-1], radius])
        ...
        radius = particle[2] * 0.8
        self.screen.blit(Game.circle_surf(self,radius, ('#DDDDDD')),
